 ...
 * @author ADmin
 */
public class ChiNhanhGUI extends javax.swing.JFrame {
    
    TaikhoanDTO taikhoan;
    TaikhoanBUS taikhoanBUS;
    
    final String[] listColumn = {"Mã chi nhánh", "Tên chi nhánh", "Địa chỉ", "Email", "Số điện thoại"};
    
    private ChinhanhBUS khachHangBUS;
    private ThemChiNhanhGUI themChiNhanhGUI;
    private SuaChiNhanhGUI suaChiNhanhGUI;
    private ChiTietChiNhanhGUI chiTietChiNhanhGUI;
    private DefaultTableModel model;

    /** Creates new form ChiNhanhGUI */
    public ChiNhanhGUI(String username) {
        khachHangBUS = new ChinhanhBUS();
        themChiNhanhGUI = new ThemChiNhanhGUI();
        suaChiNhanhGUI = new SuaChiNhanhGUI();
        chiTietChiNhanhGUI = new ChiTietChiNhanhGUI();
        initComponents();
    ...
    private void initComponents() {
        ...
        titleLabel.setText("QUẢN LÝ CHI NHÁNH");
    ...
    private void loadData() {
        model = new MyTableModel().setTable(listColumn, khachHangBUS.getDanhSachChiNhanh());
    ...
    private Vector getChiNhanhIsSelected() {
    ...
    private void xemChiTiet() {
        Vector khachHangIsSelected = getChiNhanhIsSelected();
        if (khachHangIsSelected == null) {
            showMessage("Chọn khách hàng cần xem!");
            return;
        }
        dispose();
        chiTietChiNhanhGUI.run(this, khachHangBUS, khachHangIsSelected);
    }
    
    private void suaChiNhanh() {
        Vector khachHangIsSelected = getChiNhanhIsSelected();
        if (khachHangIsSelected == null) {
        ...
        dispose();
        suaChiNhanhGUI.run(this, khachHangBUS, khachHangIsSelected);
    }
    
    private void themChiNhanh() {
        dispose();
        themChiNhanhGUI.run(this, khachHangBUS);
    }
    
    private void xoaChiNhanh() {
        Vector khachHangIsSelected = getChiNhanhIsSelected();
        if (khachHangIsSelected == null) {
            showMessage("Chọn khách hàng cần xóa!");
            return;
        }
        khachHangBUS.removeChiNhanh(khachHangIsSelected);
        loadData();
    }
    ...
    private void timKiem() {
        ...
        ketQua = khachHangBUS.getChiNhanhByMaChiNhanh(text);
        if (ketQua.size() != 0) {
            loadData(ketQua);
            System.out.println(ketQua.size());
            return;
        }
        ketQua = khachHangBUS.getDanhSachChiNhanhByTen(text);
        if (ketQua.size() != 0) {
            loadData(ketQua);
            return;
        }
        ketQua = khachHangBUS.getDanhSachChiNhanhByDiaChi(text);
        if (ketQua.size() != 0) {
            loadData(ketQua);
            return;
        }
        ketQua = khachHangBUS.getDanhSachChiNhanhByEmail(text);
        if (ketQua.size() != 0) {
            loadData(ketQua);
            return;
        }
        ketQua = khachHangBUS.getDanhSachChiNhanhBySDT(text);
        if (ketQua.size() != 0) {
            loadData(ketQua);
            return;
        }
...
//            java.util.logging.Logger.getLogger(ChiNhanhGUI.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
...
//            public void run() {
//                new ChiNhanhGUI("TK001").setVisible(true);
//            }
//        });
//    }
